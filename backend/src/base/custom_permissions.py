...
class IsAdminOrTutor(BasePermission):
    def has_permission(self, request, view):
        if request.method == 'POST':
            return request.user and request.user.is_authenticated and getattr(request.user, 'role', None) == 'tutor' or request.user and request.user.is_authenticated and request.user.is_superuser
